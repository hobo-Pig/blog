var slide = (function() {
    ...
    b.prototype = {
        init: function(c) {
            ...
            this.now = this.defaults.startIndex;//更新当前位置
            this.initBtn();
            this.liInit();
            this.bind();
            if (this.aLi.length < 2) {//处理
        ...
        bind: function() {//绑定事件到slider对象上：handleEvent事件句柄
            var g = this.oBox
              , f = b._device();
            if (!f.hasTouch) {
            ...
            var c = e ? "orientationchange" : "resize";//屏幕修改
        ...
        liInit: function() {//初始化配置所有li子节点width
            var f = this.aLi
              , h = f.length//总数
            ...
            console.log(this.defaults.fullScreen);
            if (this.defaults.fullScreen) {
                p.style.width = o * h + "px";
                console.log(this.defaults.autoHeight);
                if (this.defaults.autoHeight) {
                    ...
                    if (m) {
                        var j = new Image();
                        j.onload = function() {//设置父元素的offset
        ...
        startHandler: function(f) {
            ...
            var e = b._device()
              ...
              , c = this.now
              , h = this.aLi;
            i.addEventListener(e.moveEvt, this);//监听touchmove事件
            i.addEventListener(e.endEvt, this);//监听touchend事件
    ...
    b.extend(b, {
        ...
        getPre: function(f, c) {//@param f Number(0-c) 当前位置;//@param c Number总轮播数;
            if (f % c - 1 < 0) {
                var e = c - 1
            } else {
                var e = f % c - 1
